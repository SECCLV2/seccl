        ...
        <table>
            <tr>
                <th  colspan="14">
                    ...
                    <div style="color:#003; text-align:center; text-shadow:#666; font-size: 20px">Indicadores 2015 - Corte <?php echo $corte ?>  <br /></div></th>
            </tr>
        </table>
        <br><br>
                <table border="1">
...
on r.codigo_regional=ce.codigo_regional ORDER BY r.NOMBRE_REGIONAL ASC";
                    ...
                    while ($row2 = oci_fetch_array($statement2, OCI_BOTH))
                    {
                        ...
                        $queryCandidatosCertificados = ("SELECT COUNT(*) AS CERTIFICADOS, COUNT(UNIQUE(NRO_IDENT)) AS PERSONAS FROM CE_FIRMA_CERTIFICADOS WHERE CENTRO_ID_CENTRO = "
                                . " $row2[CODIGO_CENTRO]00 AND PERIODO = 2015 AND FECHA_CERTIFICACION <= '$corte' ");
...
WHERE PY.ID_CENTRO = $row2[CODIGO_CENTRO] AND EC.ESTADO != 0 AND EC.FECHA_REGISTRO <= '$corte' AND SUBSTR(PY.FECHA_ELABORACION, 7,4) = 2015");
